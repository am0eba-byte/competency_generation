...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!-- TO GET THE POSITION OF THE CURRENT VARIABLE ITERATION (like $pos): 
    ...
    <xsl:template name="iterateSSID">
        <xsl:param name="ssidComp" as="element()+" select="//sentenceGroup/componentSentence"/>
  
         <xsl:choose>
                                <xsl:when test="$ssidComp">
                                    <xsl:for-each-group select="$ssidComp" group-by="@id => distinct-values()">
                                        <!-- this section will be executed only once per '@id group' -->
                                        
                                        <xsl:for-each select="current-group()">
                                            <xsl:for-each select="current()">
                                                <xsl:variable name="pos" select="position()"/>
                                                <xsl:variable name="ssid" select="./@id"/>
                                                <xsl:variable name="iter" select="concat($ssid, '-', $pos)"/>
                                                <xsl:apply-templates select="$iter"/>
                                            </xsl:for-each>
                                        </xsl:for-each>
                                    </xsl:for-each-group>
                                </xsl:when>
    ...
    <xsl:template match="componentSentence">
       <!-- <xsl:param name="ssidComp" as="element()+" select="//sentenceGroup[not(.[ends-with(@id, 'sp')])]/componentSentence"/>
        -->
                    ...
                    <competency>
                        <ssID>
                           <!-- <xsl:variable name="pos">
                                <!-\-  <xsl:sequence>-\-> 
                                <xsl:value-of select="//componentSentence[@id = current()/@id]/position()"/>
                                <!-\-</xsl:sequence>-\->
                            </xsl:variable>
                            -->
                            <xsl:call-template name="iterateSSID">
                                <xsl:with-param name="ssidComp" select="."/>
                               <!-- <xsl:with-param name="posparam" select="$pos"/>-->
                            </xsl:call-template>
