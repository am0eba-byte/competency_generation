...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- 
        OUTPUT FILE PATH:
        XSLTgeneration/scope_output/K5scopes/wholenumsTIDSSID/wholenumsNestedParentOutput.xml 
    -->
            ...
            <xsl:choose>
                <xsl:when test="$param3">
                    <xsl:for-each select="$param3 ! normalize-space()">
                        <xsl:variable name="currLevel3" as="xs:string?" select="current()"/>
                        <xsl:choose>
                            <xsl:when test="$param4">                                     <!--  FOUR-PART SENTENCE -->
                                <xsl:for-each select="$param4 ! normalize-space()">
                                    <xsl:variable name="currLevel4" as="xs:string?"
                                        select="current()"/>
                                    <componentSentence>
                            ...
                            <xsl:otherwise> <!-- THREE-PART SENTENCE -->
                                <componentSentence>
                                    <xsl:element name="{$var1}">
                                        <xsl:sequence select="$currLevel1"/>
                                    </xsl:element>
                ...
                <xsl:otherwise>    <!-- TWO-PART SENTENCE -->
                    <componentSentence>
                        <xsl:element name="{$var1}">
                            <xsl:sequence select="$currLevel1"/>
                        </xsl:element>
